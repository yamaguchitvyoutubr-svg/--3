...
type DisplayMode = 'EARTHQUAKE' | 'TSUNAMI';
...
const playAlertSound = () => {
  try {
    ...
    // 3 pulses of a saw-tooth wave with pitch drop
    for (let i = 0; i < 3; i++) {
      ...
      osc2.frequency.setValueAtTime(920, startTime); // Slightly detuned
...
export const EarthquakeWidget: React.FC = () => {
  ...
  useEffect(() => {
    // If active large quake (M6+) or active tsunami, poll fast (6s)
    // Otherwise poll slow (30s)
    let isEmergency = false;

    if (quakeData && quakeData.magnitude >= 6.0) {
        isEmergency = true;
    }
    ...
    if (testMode) {
        // In test mode, we don't really poll, but let's keep it stable
    } else {
        setPollingInterval(isEmergency ? 6000 : 30000);
    }

  }, [quakeData, tsunamiData, testMode]);
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      // Fetch Quake (551) and Tsunami (552) separately to avoid 400 Bad Request
      ...
      if (quakeRes.ok) {
        ...
        if (text && text.trim().length > 0) {
            ...
            if (Array.isArray(json) && json.length > 0) {
                const latestQuake = json[0]; // Assuming limit=20 sorted by time desc
          ...
          console.warn(`Quake API Error: ${quakeRes.status}`);
      }
      ...
      if (tsunamiRes.ok) {
        ...
        if (text && text.trim().length > 0) {
          ...
          console.warn(`Tsunami API Error: ${tsunamiRes.status}`);
      }
  ...
  useEffect(() => {
      ...
      if (tsunamiData && !tsunamiData.cancelled) {
          if (tsunamiData.time !== lastAlertTime) {
              // If we haven't alerted for this specific tsunami update yet
              shouldAlert = true;
  ...
  }, [quakeData, tsunamiData, lastAlertTime, testMode]);
    ...
    if (!testMode) {
      ...
      setTsunamiData({
        ...
        areas: [
            ...
            { name: "PACIFIC COAST OF WAKAYAMA PREF", grade: "MajorWarning" },
            { name: "PACIFIC COAST OF SHIZUOKA PREF", grade: "Warning" }
        ]
      });
      ...
      setQuakeData(null);
      setTsunamiData(null);
      setLastAlertTime(null); // Reset alert memory so real alerts can fire again
  ...
  if (loading && !quakeData && !tsunamiData && !testMode) return null;
  ...
  // Determine Border/BG Color based on Active Mode & Severity
  ...
  return (
    <div className={`relative flex flex-col items-center mt-4 w-full max-w-2xl transition-all duration-500 h-28 ${(!quakeData && !tsunamiData) ? 'opacity-50' : 'opacity-100'}`}>
